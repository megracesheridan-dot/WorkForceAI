...
export function Card({
  ...
  return (
    <div
      className={`rounded-xl border border-border bg-surface p-6 shadow-sm transition-all duration-150 ease-out ${
        interactive ? "hover:-translate-y-px hover:border-border-strong hover:shadow-md" : ""
      } ${className}`}
    >
      {children}
    </div>
...
export function Badge({
  ...
  tone?: "good" | "bad" | "neutral" | "accent" | "cyan";
  children: ReactNode;
}) {
...
export function Stat({
  ...
  return (
    <div>
      ...
      <p className={`mt-1 font-mono text-2xl font-semibold tabular-nums ${valueTone[tone]}`}>{value}</p>
...
export function Button({
  ...
  const base =
    "inline-flex items-center justify-center rounded-lg px-4 py-2.5 text-sm font-medium transition-all duration-150 ease-out disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-bg active:translate-y-px";
