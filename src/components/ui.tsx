...
export function Card({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-border bg-surface p-6 transition-all duration-150 ease-out ${
        interactive ? "hover:border-border-strong hover:bg-surface-2" : ""
      } ${className}`}
    >
      {accent ? <span className="absolute inset-x-0 top-0 h-[2px] bg-accent" /> : null}
      {children}
    </div>
  );
}
...
export function Stat({
  ...
  /** @deprecated kept for API compatibility — numbers stay flat, no glow, to match the reference. */
  glow?: boolean;
}) {
  ...
  return (
    <div>
      ...
      <p className={`mt-1 font-display text-3xl font-bold tabular-nums ${valueTone[tone]}`}>{value}</p>
